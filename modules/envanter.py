...
# supabase değişkeni artık fonksiyon çağrısı ile alınacak
supabase = None  # Lazy - get_supabase_client() kullanılacak
...
# login() artık run() içinde çağrılacak
...
def save_to_supabase(df_original):
    ...
    if supabase is None:
        return 0, 0, "Supabase bağlantısı yapılandırılmamış. Secrets'a SUPABASE_KEY ekleyin."
    try:
        df = df_original.copy()
